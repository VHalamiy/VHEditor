...
//
//
//  Created by Little Yoda on 02.12.15.
//  Copyright (c) 2015 Little Yoda. All rights reserved.
//

#import "VHRotateTool.h"

@interface VHRotatePanel : UIView
...
+ (NSString*)defaultTitle {
    return NSLocalizedStringWithDefaultValue(@"VHRotateTool_DefaultTitle", nil, [VHEditorTheme bundle], @"Rotate", @"");
}
...
- (void)setup {
    ...
    _gridView = [[VHRotatePanel alloc] initWithSuperview:self.editor.imageView.superview frame:self.editor.imageView.frame];
...
- (void)executeWithCompletionBlock:(void(^)(UIImage *image, NSError *error, NSDictionary *userInfo))completionBlock {
    dispatch_async(dispatch_get_main_queue(), ^{
        UIActivityIndicatorView *indicator = [VHEditorTheme indicatorView];
...
- (void)setMenu {
    ...
    NSArray *_menu = @[
                       @{@"title":NSLocalizedStringWithDefaultValue(@"VHRotateTool_MenuItemRotateTitle", nil, [VHEditorTheme bundle], @" ", @""), @"icon":[VHEditorTheme imageNamed:[NSString stringWithFormat:@"%@/icon.png", [self class]]]},
                       @{@"title":NSLocalizedStringWithDefaultValue(@"VHRotateTool_MenuItemFlipTitle1", nil, [VHEditorTheme bundle], @" ", @""), @"icon":[VHEditorTheme imageNamed:[NSString stringWithFormat:@"%@/btn_flip1.png", [self class]]]},
                       @{@"title":NSLocalizedStringWithDefaultValue(@"VHRotateTool_MenuItemFlipTitle2", nil, [VHEditorTheme bundle], @" ", @""), @"icon":[VHEditorTheme imageNamed:[NSString stringWithFormat:@"%@/btn_flip2.png", [self class]]]},
                       ];
    
    NSInteger tag = 0;
    for(NSDictionary *obj in _menu){
        VHToolbarMenuItem *view = [VHEditorTheme menuItemWithFrame:CGRectMake(x, 0, W, H) target:self action:@selector(tappedMenu:) toolInfo:nil];
        view.tag = tag++;
