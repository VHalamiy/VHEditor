...
#import "VHEditorTheme+Private.h"
#import "UIView+Frame.h"

@implementation VHToolbarMenuItem
{
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _titleLabel.textColor = [VHEditorTheme toolbarTextColor];
        _titleLabel.font = [VHEditorTheme toolbarTextFont];
...
- (void)setSelected:(BOOL)selected
{
    if(selected != _selected){
        _selected = selected;
        if(selected){
            self.backgroundColor = [VHEditorTheme toolbarSelectedButtonColor];
        }
        else{
